...
// DMC: no se qué es ese LIMIT 10... ¿Es para cuando date_modified descenedent?
...
switch($screen) {
	...
	case "recent":
		...
		$result = mysql_query("SELECT ID FROM $database->table_name "
		                     .$obsolete
		                     ."ORDER BY date_modified DESC LIMIT 1024"
		                     );
		$tasks = array();
	...
	case "search":
		...
		if ((!isset($_REQUEST["action"]) || (isset($_REQUEST["action"]) && $_REQUEST["action"] != "search")) && empty($_REQUEST["list"])) {
			...
			if (isset($_REQUEST["list"])) {
				...
				if (isset($_REQUEST["search_date_due"]) &&
					...
					if (!empty($_REQUEST["search_date_due_before_month"]) && 
						...
						if (checkdate($_REQUEST["search_date_due_before_month"]
							...
							$query .= " AND date_due != '".$custom->minimum_date."' AND date_due <= '".$_REQUEST["search_date_due_before_year"]."-".$_REQUEST["search_date_due_before_month"]."-".$_REQUEST["search_date_due_before_day"]."'";
						}
						else {
				...
				if (isset($_REQUEST["search_date_modified"]) &&
					...
					if (!empty($_REQUEST["search_date_modified_before_month"]) && 
						...
						if (checkdate($_REQUEST["search_date_modified_before_month"]
							...
							$query .= " AND date_modified != '".$custom->minimum_date."' AND date_modified <= '".$_REQUEST["search_date_modified_before_year"]."-".$_REQUEST["search_date_modified_before_month"]."-".$_REQUEST["search_date_modified_before_day"]."'";
						}
						else {
							$page->msgs[] = $language->messages["err_search_date_before"];
						}
				...
				$query .= " AND obsolete != '1') LIMIT 1024";
				$result = mysql_query($query);
				$tasks = array();
...
// set return query string
			...
			if (count($tasks) == 1024) {
				$page->msgs[] = "This search returned more than 1024 results, only the first 1024 are shown here.";
			}
