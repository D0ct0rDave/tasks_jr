...
function print_task_edit_m($task, $screen = "focus") {
	global $_REQUEST, $root, $language, $custom;
	...
	if (empty($task->id)) {
		$task->date_due = date("Y-m-d", time() + ($custom->server_time_difference * 3600));
	}
	$temp = array(array("",""));
	for ($i = 1; $i < 13; $i++) {
...
function print_task_list_m($list, $title, $screen = "confirm", $banner = "(#)", $show_edit = 1, $show_complete = 1, $show_delete = 1, $show_new_child = 1, $show_duplicate = 0) {
	global $language, $mb_languages, $custom;
...
function print_task_view_m($task, $screen = "focus") {
	...
	if (!empty($task->date_due) && $task->date_due != $custom->minimum_date) {
