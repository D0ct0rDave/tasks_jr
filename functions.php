...
function get_friendly_timestamp($timestamp) {
	...
	$date = mktime(intval($hour), intval($min), intval($sec), intval($month), intval($day), intval($year));
...
*/
	...
	print("</a>");	
	if ($task->date_due > $custom->minimum_date || ($task->status != 0 && $task->status != 100)) {
		print("<br>");
		if ($task->status != 0 && $task->status != 100) {
			print($language->variable($language->tree["status"], $task->status));
		}
		if ($task->date_due > $custom->minimum_date && $task->status != 100) 
		{
...
function print_task_list($list = array(), $title, $screen = "confirm", $sorted_by = "", $return_query = "", $banner = "", $show_edit = 1, $show_complete = 1, $show_delete = 1, $show_new_child = 1, $show_duplicate = 0) {
	global $database, $language, $custom;
	if (count($list ?? []) > 0) {
	...
	if (count($list ?? []) == 0) {
		...
		while (list($id,$title,$notes,$status,$priority,$date_due,$container,$obsolete) = mysql_fetch_array($result)) {
			...
			if ($task->obsolete == 1) {
				...
				if ($task->date_due != $custom->minimum_date) {
					print("		<td>");
					$bold = 0;
					if ($task->status != 100) {
						$bold = 1;
...
function get_relative_date_due($date, $bold = 1) {
	...
	$now = time() + ($custom->server_time_difference * 3600);
	if (!empty($date) && $date != $custom->minimum_date) {
